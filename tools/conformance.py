...
import argparse, datetime, decimal, hashlib, html, json, os, re, shutil, subprocess, tarfile, tempfile, time, urllib.request
from pathlib import Path

ROOT=Path(__file__).resolve().parents[1]
FORMAT=ROOT.name.removeprefix("conformance-")
RESULTS=ROOT/"results/latest.json"
EXT={"json":".json","jsx":".jsx","xml":".xml","svg":".svg"}[FORMAT]
 ...
 for p in sorted(set(paths)):
  ...
  if FORMAT in ("xml","svg"):
   ...
   try: json_value(text)
    ...
    skipped[reason]=skipped.get(reason,0)+1; continue
  candidates=[(rel,text)]
  markers=list(re.finditer(r"(?m)^\s*//\s*@filename:\s*(\S+)\s*$",text))
  if markers:
   candidates=[]
   for pos,m in enumerate(markers):
    name=m.group(1); start=m.end(); end=markers[pos+1].start() if pos+1<len(markers) else len(text)
    if name.lower().endswith((".tsx",".jsx")): candidates.append((rel+"#"+name,text[start:end]))
  for source_name,source_text in candidates:
   ident=hashlib.sha256((source_name+"\0"+source_text).encode()).hexdigest()[:16]
   rows.append({"id":ident,"suite":next(iter(load(ROOT/"config/sources.json"))),"source":source_name,"text":source_text})
 parsed=jsx_values([r["text"] for r in rows])
 selected=[]
 for row,result in zip(rows,parsed):
  if result["ok"]: selected.append(row)
  else: skipped["source-parser-rejected"]=skipped.get("source-parser-rejected",0)+1
 rows=selected[:limit or None]
...
def compare(before,after):
 try:
  if FORMAT=="json": a,b=json_value(before),json_value(after)
   ...
   return compare_jsx_results(a,b)
 except Exception as e:
  try:
...
def compare_jsx_results(before,after):
 if not before["ok"]: return "source-rejected",{"before":before}
 if not after["ok"]: return "parser-rejected",{"after":after}
 if before["tokens"]!=after["tokens"]: return "token-difference",{"before":before,"after":after}
 # JSX text is a semantic token: the scanner tokenizes text words as
 # identifiers with separating whitespace as trivia, so the non-trivia token
 # stream alone cannot detect `a  b` -> `a b`. Compare every JsxText node's
 # exact text in document order as a second, stricter projection.
 if before.get("jsxTexts")!=after.get("jsxTexts"): return "token-difference",{"before":before,"after":after}
 return "pass",{}
...
def execute(cases_path,exe,result):
 ...
 jsx_before=jsx_after=None
 if FORMAT=="jsx":
  jsx_before=jsx_values([c["text"] for c in cases])
  jsx_after=jsx_values([outputs.get(c["id"],"") for c in cases])
 for pos,c in enumerate(cases):
  if c["id"] in errors: status,evidence="minify-error",{"error":errors[c["id"]]}
  elif FORMAT=="jsx": status,evidence=compare_jsx_results(jsx_before[pos],jsx_after[pos])
